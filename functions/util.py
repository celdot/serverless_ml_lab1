...
def backfill_predictions_for_monitoring(weather_fg, air_quality_df, monitor_fg, model):
    ...
    features_df = pd.concat([air_quality_df,  features_df])
    features_df = features_df.sort_values(by=['date'], ascending=True)
    features_df = features_df.tail(10)
    features_df['predicted_pm25'] = model.predict(features_df[['pm25_3days', 'temperature_2m_mean', 'precipitation_sum', 'wind_speed_10m_max', 'wind_direction_10m_dominant']])
    df = pd.merge(features_df, air_quality_df[['date','pm25','street','country']], on="date", suffixes=('', '_airquality'))[['date', 'pm25', 'city', 'country', 'street', 'pm25_3days',
       'temperature_2m_mean', 'precipitation_sum', 'wind_speed_10m_max',
       'wind_direction_10m_dominant', 'predicted_pm25']]
